...
class PacketExtractor(object):
  ...
  def __init__(self, udid=None):
    ...
    self.conn.print_stats = self._print_stats
  ...
  def _print_stats(self):
    stderr_print("\nNetwork process stats ({})\n{}".format(datetime.now(), "-"*80) )
    stderr_print("{:16} | {:<4} | {:<4} | {:<8} | {:<20} | {:<20}".format("pname","pid","pkts","bytes","first seen","last seen"))
    netstat = dict(sorted(self._netstat.items(), key = lambda x: x[1]["packets"], reverse=True ))
    for pid,pname in netstat:
      pkey = (pid,pname)
      ptat = netstat[pkey]
      if pname == "":
        pname = "[System]"
      stderr_print("{:16} | {:<4} | {:<4} | {:<8} | {:<20} | {:<20}".format( 
                      pname, 
                      pid, 
                      ptat["packets"],
                      ptat["bytes"],
                      str(datetime.fromtimestamp(int(ptat["fseen"]))),
                      str(datetime.fromtimestamp(int(ptat["lseen"])))
                      )
                  )
      sys.stderr.flush()
  ...
  def _chunk_to_packet(self, chunk):
    ...
    if if_type == 0xFF:
      ...
      raise ValueError('unknown interface type {}'.format(hex(if_type)))

    def update_proctuple_netstat(self, ptuple):
      if ptuple in self._netstat:
        self._netstat[ptuple]["packets"] += 1
        self._netstat[ptuple]["bytes"] += payload_len
        self._netstat[ptuple]["lseen"] = time.time()
      else:
        tm = time.time()
        self._netstat[ptuple] = {"packets": 1, "bytes": payload_len, "fseen": tm, "lseen": tm}
    update_proctuple_netstat(self,(pid,comm))
    update_proctuple_netstat(self,(epid,ecomm))
...
class NGPacketDumper(PacketDumper):
  ...
  def run(self, packet_cb=None):
    # write section block
    self._write_block(0x0A0D0D0A, self.SECTION_BLOCK_STRUCT.pack(0x1A2B3C4D, 1, 0, -1))
    # process packets
    if_name_to_idx = {}
    next_if_idx = 0
  ...
  def run(self, packet_cb=None):
    ...
    for pkt in self.pkt_iter:
      ...
      custom_options = {
        ...
        0x8002: UM32.pack(pkt.svc_class),
        0x0002: UM32.pack(self.IN_OUT_TO_EPBFLAGS[pkt.in_out])
      }
      if (eproctuple_idx != proctuple_idx) and ( eprocid > 0 ):
        custom_options[0x8003] = UM32.pack(eproctuple_idx)

      self._write_block(6, pcap_hdr + pkt.pkt_payload, custom_options)
